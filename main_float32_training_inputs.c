/*
    ...
    This version expects training inputs of float32 between -1 and 1
    and a song input of unsigned 8-bit which then outputs an unsigned 8-bit
    output of the transformed input.
*/
...
unsigned char quantise_float(f32 f)
{
...
int main()
{
    ...
    {
        f32 epoch_loss = 0.f;
        const uint TTS = DSS/sizeof(f32);
        const uint DSS4 = TTS-4;
        for(uint i=4; i < DSS4; i++)
        {
            const int ofs = i-4;
            const f32 loss = processNetwork(&net, &train_x[ofs+j], &train_y[ofs], NULL);
            epoch_loss += loss;
